...
| 1.4 | [UDP](#14-udp) | Done |
| 1.5 | [IP Addressing/Subnetting](#15-ip-addressingsubnetting) | Done |
| 1.6 | [CIDR](#16-cidr) | Done |
| 1.7 | [DNS](#17-dns) | Done |
| 1.8 | [DNS Resolution](#18-dns-resolution) | Done |
| 1.9 | [HTTP/HTTPS](#19-httphttps) | Done |
| 1.10 | [SSL/TLS](#110-ssltls) | Done |
| 1.11 | [HTTP2 & HTTP3](#111-http2-http3) | Done |
| 1.12 | [QUIC](#112-quic) | Done |
| 1.13 | [Forward & Reverse Proxy](#113-forward-reverse-proxy) | Done |
| 1.14 | [NAT](#114-nat) | Done |
| 1.15 | [VPN](#115-vpn) | Done |
| 1.16 | [SSE & Polling & Websocets](#116-sse-polling-websocets) | Done |
| 1.17 | [CDN](#117-cdn) | Done |
| 1.18 | [Anycast/Multicast/Broadcast](#118-anycastmulticastbroadcast) | Done |
| 1.19 | [Load Balancer](#119-load-balancer) | Done |
| 1.20 | [Load Balancer Algorithm](#120-load-balancer-algorithm) | Done |
| 1.21 | [MTU](#121-mtu) | Done |
| 1.22 | [Keep Alive Connections](#122-keep-alive-connections) | Done |
...
| Topic | Core idea | Key protocol / tool |
|-------|-----------|---------------------|
| OSI model | 7-layer reference framework | Layers 4–7 most cited |
| TCP/IP | Practical internet stack | IPv4, TCP, UDP |
| TCP handshake | SYN → SYN-ACK → ACK | 1 RTT connection setup |
| UDP | Connectionless datagrams | DNS, QUIC base |
| Subnetting | Split network/host bits | /24, private RFC1918 |
| CIDR | address/prefix notation | Security group rules |
| DNS | Name → IP mapping | A, CNAME, TTL |
| DNS resolution | Recursive lookup chain | Caching at each hop |
| HTTP/HTTPS | Request/response web protocol | TLS for HTTPS |
| TLS | Encryption + authentication | TLS 1.3, certificates |
| HTTP/2 | Multiplexed streams on TCP | HPACK, binary frames |
| HTTP/3 | HTTP over QUIC/UDP | No TCP HOL blocking |
| QUIC | UDP transport with TLS + streams | Connection migration |
| Reverse proxy | Server-facing gateway | nginx, ALB |
| NAT | Private → public IP mapping | NAT gateway |
| VPN | Encrypted tunnel | WireGuard, IPsec |
| SSE/Polling/WS | Real-time client updates | WebSocket bidirectional |
| CDN | Edge caching globally | CloudFront, Cloudflare |
| Anycast | Route to nearest IP | CDN, DNS resolvers |
| Load balancer | Traffic distribution | L4 vs L7 |
| LB algorithm | Backend selection logic | RR, least conn, hash |
| MTU | Max packet size | 1500 Ethernet, PMTUD |
| Keep-alive | Reuse TCP connections | HTTP/1.1 default |
