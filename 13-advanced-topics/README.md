...
| # | Sub-topic | Status |
|---|-----------|--------|
| 13.1 | [Bloom Filters](#131-bloom-filters) | Documented |
| 13.2 | [HyperLogLog](#132-hyperloglog) | Documented |
| 13.3 | [Count Min Sketch](#133-count-min-sketch) | Documented |
| 13.4 | [Merkle Trees](#134-merkle-trees) | Documented |
| 13.5 | [Skip Lists](#135-skip-lists) | Documented |
| 13.6 | [Trie](#136-trie) | Documented |
| 13.7 | [Distributed Hash Tables](#137-distributed-hash-tables) | Documented |
| 13.8 | [UUID](#138-uuid) | Documented |
| 13.9 | [Snowflake IDs](#139-snowflake-ids) | Documented |
| 13.10 | [ULID](#1310-ulid) | Documented |
| 13.11 | [KSUID](#1311-ksuid) | Documented |
...
flowchart TB
    subgraph Insert["Insert 'user:42'"]
        H1["hash₁ → bit 3"] --> B[(Bit Array m bits)]
        H2["hash₂ → bit 17"] --> B
        H3["hash₃ → bit 91"] --> B
    end
...
- Optimal `k ≈ (m/n) · ln 2` where `n` = expected elements; false-positive rate `p ≈ (1 − e^(−kn/m))^k`.
...
- **41 bits time** ≈ 69 years from epoch; plan epoch and overflow migration early.
...
| Structure | Answers | Memory | Error type |
|-----------|---------|--------|------------|
| Bloom filter | "Maybe in set?" | Fixed bits | False positives |
| HyperLogLog | "How many unique?" | ~12 KB | ~2% relative |
| Count-Min Sketch | "How many times?" | Fixed counters | Overestimate |
| Merkle tree | "Dataset integrity?" | O(n) hashes | Exact |
| Skip list | Ordered set ops | O(n) pointers | Exact |
| Trie | Prefix match | O(total chars) | Exact |
| DHT | "Who owns key?" | Per-node state | Depends on replication |
| Snowflake/ULID/KSUID | Unique sortable ID | 8–20 bytes | None (if clock OK) |
